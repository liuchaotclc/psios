...
+ (PSLayer *) layer
{
    PSLayer *layer = [[PSLayer alloc] init];
    return layer;
}
...
- (void) setBlendMode:(PSBlendMode)blendMode
{
...
- (void) setColorBalance:(PSColorBalance *)colorBalance
{
...
- (void) drawFragment:(PSPaintingFragment *)fragment
{
    PSPaintingFragment *inverseFragment = [fragment inverseFragment:self];
...
- (void) registerUndoInRect:(CGRect)rect
{
    ...
    PSPaintingFragment *fragment = [PSPaintingFragment paintingFragmentWithData:[self imageDataInRect:rect] bounds:rect];
...
- (UIImage *) thumbnail
{
    if (!thumbnail_) {
        ...
        if (status == GL_FRAMEBUFFER_COMPLETE) {
            ...
            PSShader *blitShader = [self.painting getShader:@"blit"];
...
- (void) encodeWithPSCoder:(id<PSCoder>)coder deep:(BOOL)deep
{
    ...
    if (deep) {
        ...
        //        [self saveImage];
        PSTypedData *image = [PSTypedData data:data mediaType:@"image/x-brushes-layer" compress:YES uuid:self.uuid isSaved:wasSaved];
...
- (void) updateWithPSDecoder:(id<PSDecoder>)decoder deep:(BOOL)deep
{
    ...
    PSBlendMode blendMode = (PSBlendMode) [decoder decodeIntegerForKey:WDBlendModeKey];
...
{
    switch (blendMode_) {
        case PSBlendModeMultiply:
            glBlendFunc(GL_DST_COLOR, GL_ONE_MINUS_SRC_ALPHA);
            break;
        case PSBlendModeScreen:
            glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_COLOR);
            break;
        case PSBlendModeExclusion:
...
- (void) setHueSaturation:(PSHueSaturation *)inHueSaturation
{
    if (!hueSaturation_ && inHueSaturation) {
        ...
        if (status == GL_FRAMEBUFFER_COMPLETE) {
            ...
            PSShader *blitShader = [self.painting getShader:@"toHueChromaLuma"];
            glUseProgram(blitShader.program);
...
- (void) blit:(GLfloat *)proj withTransform:(CGAffineTransform)tX
{
    ...
    if (self.clipWhenTransformed) {
        ...
        PSGLRenderInRect(rect, CGAffineTransformIdentity);
...
- (void) blit:(GLfloat *)proj withMask:(GLuint)maskTexture color:(PSColor *)color
{
    // use shader program
    PSShader *blitShader = [self.painting getShader:@"blitWithMask"];
    glUseProgram(blitShader.program);
...
{
    // use shader program
    PSShader *blitShader = [self.painting getShader:@"blitWithEraseMask"];
    glUseProgram(blitShader.program);
...
- (void) duplicateLayer:(PSLayer *)layer copyThumbnail:(BOOL)copyThumbnail
{
...
- (void) merge:(PSLayer *)layer
{
    [self modifyWithBlock:^{
        PSShader *blitShader = [self.painting getShader:@"merge"];
        glUseProgram(blitShader.program);
...
- (void) commitStroke:(CGRect)bounds color:(PSColor *)color erase:(BOOL)erase undoable:(BOOL)undoable path:(PSPath *)path
{
    if (undoable) {
    ...
    [self modifyWithBlock:^{
        ...
        PSShader *shader = erase ? [self.painting getShader:@"compositeWithEraseMask"] : [self.painting getShader:@"compositeWithMask"];
...
- (void) testCommitStroke:(CGRect)bounds color:(PSColor *)color erase:(BOOL)erase index:(NSInteger)index total:(NSInteger)total
{
...
- (void) fill:(PSColor *)color
{
    if (color.alpha < 1.0 || self.alphaLocked) {
        [self modifyWithBlock:^{
            ...
            PSShader *shader = [self.painting getShader:@"fill"];
            glUseProgram(shader.program);
...
- (void) applyShader:(PSShader *)shader undoBits:(BOOL)undo
{
    [self modifyWithBlock:^{
        glClear(GL_COLOR_BUFFER_BIT);
...
- (void) renderImage:(UIImage *)image transform:(CGAffineTransform)inTransform
{
    ...
    PSTexture *imageTexture = [PSTexture textureWithImage:image];
    
    [self modifyWithBlock:^{
        PSShader *blitShader = hasAlpha ? [self.painting getShader:@"unPremultipliedBlit"] : [self.painting getShader:@"nonPremultipliedBlit"];
        glUseProgram(blitShader.program);
        ...
        PSGLRenderInRect(rect, inTransform);
...
- (void) freeze
{
    fragment_ = [PSPaintingFragment paintingFragmentWithData:self.imageData bounds:self.painting.bounds];
    [self freeGLResources];
}
