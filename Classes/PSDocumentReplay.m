...
#import "PSReplayUndoAnalyzer.h"
#import "PSSleepTimer.h"
#import "PSUtilities.h"

@implementation PSDocumentReplay {
    BOOL includeUndos_;
    NSSet *undone_;
    NSArray *history_;
    CGSize size_;
...
- (id) initWithDocument:(PSDocument *)document includeUndos:(BOOL)undos scale:(float)scale
{
    ...
    [self optimizeUndos];
    scale_ = scale;
    size_ = PSMultiplySizeScalar(document.painting.dimensions, scale);
...
- (void) dealloc
{
    ...
    [[PSSleepTimer sharedInstance] enableTimer:self];
}

- (void) optimizeUndos
{
    PSReplayUndoAnalyzer *analyzer = [[PSReplayUndoAnalyzer alloc] init];
...
{
    [self pause];
    self.painting = [[PSPainting alloc] initWithSize:size_];
...
{
    if (self.paused) {
        [[PSSleepTimer sharedInstance] disableTimer:self];
        self.paused = NO;
...
- (id<PSDocumentChange>) nextChange
{
    if (changeNumber_ < history_.count) {
        ...
        } else if ([decoded conformsToProtocol:@protocol(PSDocumentChange)]) {
            if (scale_ != 1.f) {
...
- (void) saveConversion
{
    [[PSPaintingManager sharedInstance] installPainting:self.painting withName:nil initializer:^(PSDocument *document) {
