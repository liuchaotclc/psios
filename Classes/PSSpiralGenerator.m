...
@implementation PSSpiralGenerator
...
- (void) drawSpiralAtPoint:(CGPoint)center radius:(float)radius inContext:(CGContextRef)ctx randomizer:(PSRandom *)random
{ 
    int         segments = 15;
    float       decay = 80;
    ...
    for (int segment = 0; segment <= segments; segment++) {
        ...
        CGPoint P2 = PsSubtractPoints(P3, PSMultiplyPointScalar(CGPointMake(xPrime, yPrime), deltaT));
        CGPoint P1 = PSAddPoints(P3, PSMultiplyPointScalar(CGPointMake(xPrime, yPrime), deltaT));
