...
- (void) loadShaders
{
    ...
    for (NSString *key in shaderDict.keyEnumerator) {
        ...
        PSShader *shader = [[PSShader alloc] initWithVertexShader:vertex
...
- (void) setActiveLayer:(PSLayer *)layer
{
    if (!layer) {
...
- (PSLayer *) layerWithUUID:(NSString *)layerUUID
{
    for (PSLayer *layer in layers_) {
...
- (BOOL) canMergeDown
{
    ...
    PSLayer *bottomLayer = layers_[(self.indexOfActiveLayer - 1)];
...
- (void) mergeDown
{
    ...
    PSLayer *bottom = layers_[(self.indexOfActiveLayer - 1)];
    PSLayer *top = self.activeLayer;
...
- (void) duplicateActiveLayer
{
    ...
    PSLayer *layer = self.activeLayer;
    changeDocument(self, [PSAddLayer addLayerAtIndex:self.indexOfActiveLayer+1]);
    changeDocument(self, [PSDuplicateLayer duplicateLayer:layer toLayer:self.activeLayer]);
...
{
    PSLayer *layerToDelete = self.activeLayer;
    if (layers_.count == 1) {
        ...
        changeDocument(self, [PSAddLayer addLayerAtIndex:1]);
    }
...
- (void) moveLayer:(PSLayer *)layer toIndex:(NSUInteger)dest
{
    ...
    PSLayer *expectedActiveLayer = self.activeLayer;
...
- (NSData *) imageDataWithSize:(CGSize)size backgroundColor:(UIColor *)color
{
    ...
    if (status == GL_FRAMEBUFFER_COMPLETE) {  
        ...
        // blit each layer
        for (PSLayer *layer in self.layers) {
            if (layer.visible) {
...
{
    PSPainting *painting = [[PSPainting alloc] init];
...
- (GLuint) quadVAO
{
    if (!quadVAO_) {
        ...
        PSGLBuildQuadForRect(bounds, CGAffineTransformIdentity, &quadVAO_, &quadVBO_);
    }
...
- (GLuint) activePaintTexture
{
    if (!activePaintTexture_) {
        activePaintTexture_ = [self generateTexture:nil deepColor:[PSPainting supportsDeepColor]];
    }
...
- (PSTexture *) brushTexture:(PSBrush *)brush
{
    [EAGLContext setCurrentContext:self.context];
    
    if (!brushTexture_ || (brush != lastBrush_)) {
        PSStampGenerator *gen = brush.generator;
        
        if (brushTexture_) {
            [brushTexture_ freeGLResources];
        }
        
        brushTexture_ = [PSTexture alphaTextureWithImage:gen.stamp];
        lastBrush_ = brush;
    }
...
{
    PSShader *brushShader = [self getShader:@"brush"];
...
{
    // use shader program
    PSShader *blitShader = [self getShader:@"straightBlit"];
