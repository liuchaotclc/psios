...
- (void)viewWillLayoutSubviews{
    self.view.frame = CGRectMake(0, 0, 260, 400);
...
- (void) updateCellIndices
{
    dispatch_async(dispatch_get_main_queue(), ^{
        for (PSLayerCell *cell in layerTable_.visibleCells) {
            [cell updateIndex];
        }
    });
}
...
- (void) layerVisibilityChanged:(NSNotification *)aNotification
{
    ...
    PSLayerCell *layerCell = (PSLayerCell *) [layerTable_ cellForRowAtIndexPath:indexPath];
...
- (void) opacitySliderMoved:(PSColorSlider *)sender
{
    ...
    PSLayerCell *cell = (PSLayerCell *) [layerTable_ cellForRowAtIndexPath:[layerTable_ indexPathForSelectedRow]];
...
{
    self.blendModeSheet = [PSActionSheet sheet];
    
    __unsafe_unretained PSLayerController *layerController = self;
    
    for (NSNumber *mode in PSBlendModes()) {
        [blendModeSheet addButtonWithTitle:WDDisplayNameForBlendMode((PSBlendMode)mode.integerValue)
                                    action:^(id sender) {
                                        [layerController setBlendMode:(PSBlendMode)mode.integerValue forLayer:layer];
                                    }];
    }
...
@implementation PSLayerController (Private)
