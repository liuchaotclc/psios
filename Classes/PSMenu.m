...
@interface PSMenu (Private)
- (void) createLabels_;
@end

@implementation PSMenu
...
- (id) initWithItems:(NSArray *)items
{
    ...
    self.opaque = NO;
    self.backgroundColor = PSUseModernAppearance() ? nil : [UIColor whiteColor];
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (selectedIndex_ >= 0) {
        PSMenuItem *item = items_[selectedIndex_];
...
- (void) createLabels_
{
    ...
    for (PSMenuItem *item in items_) {
        ...
        if (item.image) {
            ...
            imageFrame.origin = PSRoundPoint(imageFrame.origin);
