...
@interface PSPaintingManager (Private)
- (void) createNecessaryDirectories_;
- (void) savePaintingOrder_;
@end

@implementation PSPaintingManager

+ (PSPaintingManager *) sharedInstance
{
    static PSPaintingManager *shared = nil;

    @synchronized (self) {
        if (!shared) {
            shared = [[PSPaintingManager alloc] init];
        }
    }
...
- (void) installPainting:(PSPainting *)painting withName:(NSString *)paintingName
               ...
               afterSave:(void (^)(PSDocument *document))afterSave
{
    ...
    PSDocument *document = [[PSDocument alloc] initWithFileURL:url painting:painting];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
           ...
           [document openWithCompletionHandler:^(BOOL success) {
               if (initializer) {
                   ...
                   changeDocument(painting, [PSClearUndoStack clearUndoStack]);
               }
               if (afterSave) {
...
afterSave:(void (^)(PSDocument *document))afterSave
{
    ...
    PSPainting *painting = [[PSPainting alloc] initWithSize:image.size];
    NSString *paintingName = [self uniqueFilenameWithPrefix:imageName];
    [self installPainting:painting withName:paintingName initializer:^(PSDocument *document){
        // create initial layer with image
        changeDocument(painting, [PSAddImage addImage:image atIndex:0 mergeDown:NO transform:CGAffineTransformIdentity]);
    } afterSave:afterSave];
//     ^(WDDocument *document) {
//        if (afterSave) {
//            // do something with the document we created
...
{   
    PSPainting *painting = [[PSPainting alloc] initWithSize:size];
    [self installPainting:painting withName:[self uniqueFilename]
              initializer:^(PSDocument *document) {
                  // create initial layer
                  changeDocument(painting, [PSAddLayer addLayerAtIndex:0]);
              }
                afterSave:afterSave];
}
...
{
    NSURL *url = [self urlForName:name];
    PSDocument *document = [[PSDocument alloc] initWithFileURL:url];
...
- (PSDocument *) duplicatePainting:(PSDocument *)document
{ 
    ...
    PSDocument *duplicate = [[PSDocument alloc] initWithFileURL:url painting:document.painting];
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (NSString *) installPaintingFromURL:(NSURL *)url error:(NSError **)outError
{
    ...
    if (files && files.count) {
        ...
        PSPainting *painting = [coder decodeRoot:@"painting.json" from:files][@"painting"];
        if (!painting) {
...
@implementation PSPaintingManager (Private)
