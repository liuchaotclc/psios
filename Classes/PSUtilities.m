...
CGSize PSSizeOfRectWithAngle(CGRect rect, float angle, CGPoint *upperLeft, CGPoint *upperRight)
{
    ...
    if (upperLeft) {
        *upperLeft = PsSubtractPoints(corners[0], center);
    }
    
    if (upperRight) {
        *upperRight = PsSubtractPoints(corners[3], center);
    }
...
CGPoint PSNormalizePoint(CGPoint vector)
{
    ...
    return PSMultiplyPointScalar(vector, 1.0f / WDDistance(CGPointZero, vector));
}
...
CGPoint PSSharpPointInContext(CGPoint pt, CGContextRef ctx)
{
    ...
    pt = PSFloorPoint(pt);
    pt = PSAddPoints(pt, CGPointMake(0.5f, 0.5f));
    pt = CGContextConvertPointToUserSpace(ctx, pt);
    
    return pt;
}

CGPoint PSConstrainPoint(CGPoint delta)
{
...
void convertQuadraticPathElement(void *info, const CGPathElement *element)
{
    ...
    switch (element->type) {
        ...
        case kCGPathElementAddQuadCurveToPoint:
            ...
            CGPoint outPoint = PSAddPoints(prev, PSMultiplyPointScalar(PsSubtractPoints(element->points[0], prev), 2.0f / 3));
            CGPoint inPoint = PSAddPoints(element->points[1], PSMultiplyPointScalar(PsSubtractPoints(element->points[0], element->points[1]), 2.0f / 3));
...
BOOL PSLineInRect(CGPoint a, CGPoint b, CGRect test)
{
    ...
    if(acode == 0 || bcode == 0) { // one or both endpoints within rect
        ...
        if (PSLineInRect(a, middle, test)) {
            return YES;
        }
        
        if (PSLineInRect(middle, b, test)) {
            return YES;
        }
        return NO;
    }
...
BOOL PSLineSegmentsIntersectWithValues(CGPoint A, CGPoint B, CGPoint C, CGPoint D, float *rV, float *sV) 
{
...
{
    return PSLineSegmentsIntersectWithValues(A, B, C, D, NULL, NULL);
}
...
{
    return min + PSRandomFloat() * (max - min);
}
...
PSQuad PSQuadNull()
{
    ...
    return PSQuadMake(bogusPoint, bogusPoint, bogusPoint, bogusPoint);
}
...
{
    PSQuad nullQuad = PSQuadNull();
    if (PSQuadEqualToQuad(a, nullQuad) || PSQuadEqualToQuad(b, nullQuad)) {
        return NO;
    }
    
    for (int i = 0; i < 4; i++) {
        for (int n = 0; n < 4; n++) {
            if (PSLineSegmentsIntersect(a.corners[i], a.corners[(i+1)%4], b.corners[n], b.corners[(n+1)%4])) {
                return YES;
            }
        }
    }
...
void PSLogTiming(NSString *message)
{
...
{
    return (PSDeviceIsPhone() && [UIScreen mainScreen].bounds.size.height == 568) ? YES : NO;
}
...
BOOL PSCanUseScissorTest()
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
            ...
            CGSize size = PSMultiplySizeScalar([UIScreen mainScreen].bounds.size, [UIScreen mainScreen].scale);
