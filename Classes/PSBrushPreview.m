...
{
    static PSBrushPreview *preview_ = nil;
    
    if (!preview_) {
        preview_ = [[PSBrushPreview alloc] init];
    }
...
- (void) buildPath
{
    ...
    // build a nice little sin curve
    {
        ...
        for (int i = 0; i < kNumLineSegments; i++) {
            ...
            node = [PSBezierNode bezierNodeWithAnchorPoint:[PS3DPoint pointWithX:pt.x y:pt.y z:fraction]];
            [nodes addObject:node];
        }
    }
    
    self.path = [[PSPath alloc] init];
...
- (id) init
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(brushGeneratorChanged:)
                                                 name:PSBrushGeneratorChanged
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(brushGeneratorChanged:)
                                                 name:PSBrushGeneratorReplaced
                                               object:nil];
    
    return self;
}

- (PSShader *) brushShader
{
    if (!brushShader) {
        NSArray *attributes = @[@"inPosition", @"inTexcoord", @"alpha"];
        NSArray *uniforms = @[@"modelViewProjectionMatrix", @"texture", @"noise"];
        
        brushShader = [[PSShader alloc] initWithVertexShader:@"brush" fragmentShader:@"brush" attributesNames:attributes uniformNames:uniforms];
    }
    
    return brushShader;
}

- (PSTexture *) brushTexture
{
    if (!brushTexture) {
        PSStampGenerator *gen = brush.generator;
        brushTexture = [PSTexture alphaTextureWithImage:gen.smallStamp];
    }
...
- (UIImage *) previewWithSize:(CGSize)size
{
    ...
    size = PSMultiplySizeScalar(size, [UIScreen mainScreen].scale);
