...
{
    PSBarItem *item = [[PSBarItem alloc] init];
    item.view = view;
...
+ (PSBarItem *) barItemWithView:(UIView *)view landscapeView:(UIView *)landscapeView
{
    PSBarItem *item = [[PSBarItem alloc] init];
    item.view = view;
...
+ (UIImage *) addShadowToImage:(UIImage *)inImage
{
    ...
    CGSize  size = PSAddSizes(inImage.size, CGSizeMake(4,4)); // add a small buffer for the shadow
...
+ (PSBarItem *) barItemWithImage:(UIImage *)inImage target:(id)target action:(SEL)action
{
    UIButton *button = nil;
    UIButton *landscapeButton = nil;
    
    inImage = [self addShadowToImage:inImage];
    button = [PSBarItem buttonWithImage:inImage height:kWDDefaultBarHeight];
    [button addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    
    return [PSBarItem barItemWithView:button landscapeView:landscapeButton];
}
...
- (void) setImage:(UIImage *)inImage
{
    if (self.view && [self.view isKindOfClass:[UIButton class]]) {
        ...
        [button setImage:[PSBarItem addShadowToImage:inImage] forState:UIControlStateNormal];
    }
}
...
+ (PSBar *) bottomBar
{
    PSBar *bar = [[PSBar alloc] initWithFrame:CGRectMake(0, 0, kWDDefaultBarHeight, kWDDefaultBarHeight)];
...
- (void) setItems:(NSArray *)inItems animated:(BOOL)animated
{
    ...
    // remove old cells
    for (PSBarItem *item in self.items) {
...
- (void) layoutSubviews
{
    ...
    if (flexibleContentItems.count != 0) {
        ...
        for (PSBarItem *item in flexibleContentItems) {
    ...
    for (PSBarItem *item in self.items) {
        if (item.activeView) {
            ...
            frame.origin = PSRoundPoint(currentOrigin);
...
- (void) setOrientation:(UIInterfaceOrientation)orientation
{
    ...
    float height = kWDDefaultBarHeight;
    ...
    self.titleLabel.font = [self portraitFont];
