...
#import "PSGLUtilities.h"
#import "PSLabel.h"
#import "PSLayer.h"
#import "PSModifyLayer.h"
#import "PSPanGestureRecognizer.h"
#import "PSTransformOverlay.h"
#import "PSTool.h"
#import "PSShader.h"
#import "PSShadowQuad.h"
#import "PSTexture.h"
...
//    [self addGestureRecognizer:pinchGesture];
    ...
    PSPanGestureRecognizer *panGesture = [[PSPanGestureRecognizer alloc]
...
- (void) setPainting:(PSPainting *)inPainting
{
...
- (PSColor *) colorAtPoint:(CGPoint)pt
{
    ...
    return [PSColor colorWithRed:(pixel[0] / 255.0f) + (1.0f - alpha)
...
//    }
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        PSColor *currentColor = [PSActiveState sharedInstance].paintColor;
        PSColor *pickedColor = [[self colorAtPoint:docLoc] colorWithAlphaComponent:currentColor.alpha];
        [PSActiveState sharedInstance].paintColor = pickedColor;
...
- (void) displayEyedropperAtPoint:(CGPoint)pt 
{
    ...
    eyedropper_ = [[PSEyedropper alloc] initWithFrame:CGRectMake(0, 0, kDropperRadius * 2, kDropperRadius * 2)];
...
- (void) adjustForReplayScale:(float)scale
{
    if (scale != 1.0f) {
        ...
        userSpacePivot_ = PSMultiplyPointScalar(userSpacePivot_, scale);
...
- (float) fitScale
{
    CGRect  bounds = PSMultiplyRectScalar(self.superview.bounds, [UIScreen mainScreen].scale);
...
- (BOOL) isScaledToFit
{
    ...
    if (!CGPointEqualToPoint(deviceSpacePivot_, PSCenterOfRect(self.bounds))) {
        return NO;
    }
...
- (void) scaleDocumentToFit:(BOOL)animated
{
    ...
    deviceSpacePivot_ = PSCenterOfRect(self.bounds);
...
{
    PSShader *blitShader = [self.painting getShader:@"nonPremultipliedBlit"];
    ...
    PSGLRenderInRect(rect, transform);
    WDCheckGLError();
}
...
//    canvasTransform_ = CGAffineTransformMakeScale(1.0/3, 1.0/3);
    ...
    if (PSCanUseScissorTest()) {
        CGRect scissorRect = [self convertRectFromDocument:rect];
        scissorRect = PSMultiplyRectScalar(scissorRect, scale);
...
//    }
    ...
    if (DEBUG_DIRTY_RECTS) {
        PSColor *randomColor = [PSColor randomColor];
...
- (void)testDraw
{
    PSLayer *firstLayer = self.painting.layers.firstObject;
...
{
    PSShader *blitShader = [self.painting getShader:@"simple"];
...
- (NSArray *) shadowSegments
{
    if (!shadowSegments) {
        ...
        for (NSString *imageName in images) {
            PSShadowQuad *quad = [PSShadowQuad imageQuadWithImage:[UIImage imageNamed:imageName]
...
- (void)setContext:(EAGLContext *)context
{
    ...
    if (context_) {
        mainRegion = [PSGLRegion regionWithContext:context_ andLayer:(CAEAGLLayer *) self.layer];
        [mainRegion resize];
    }
}
...
{
    userSpacePivot_ = PSAddPoints(userSpacePivot_, delta);
}
...
- (void) setFrame:(CGRect)frame
{
    if (self.painting) {
        ...
        deviceSpacePivot_ = PSCenterOfRect(bounds);
...
{
    userSpacePivot_ = [self convertPointToDocument:PSCenterOfRect(self.bounds)];
}
...
{
    deviceSpacePivot_ = PSAddPoints(deviceSpacePivot_, delta);
    [self rebuildViewTransform_];
}
...
{
    if (autoHide && !PSDeviceIsPhone() && !self.controller.interfaceHidden) {
        return;
    }
...
- (void) animateViewToScale:(float)endScale
{
    ...
    CGPoint endDevicePivot = PSCenterOfRect(self.bounds);
    CGPoint deviceDelta = PsSubtractPoints(endDevicePivot, startDevicePivot);
    ...
    for (float t = step; t < 1.0f; t += step) {
        float interpT = PSSineCurve(t);
        scale_ = startScale + (interpT * scaleDelta);
        deviceSpacePivot_ = PSAddPoints(startDevicePivot, PSMultiplyPointScalar(deviceDelta, interpT));
        [self rebuildViewTransform_];
    }
...
- (void) doScaleAnimationFrom:(float)start to:(float)end
{
    ...
    for (float t = step; t < 1.0f; t += step) {
        scale_ = start + PSSineCurve(t) * delta;
        [self rebuildViewTransform_];
    }
...
- (void) handlePinchGesture:(UIPinchGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        if (sender.numberOfTouches != lastTouchCount_) {
            correction_ = PsSubtractPoints(deviceSpacePivot_, flipped);
            lastTouchCount_ = sender.numberOfTouches;
        }
        
        deviceSpacePivot_ = PSAddPoints(flipped, correction_);
...
- (void) oneTap:(UITapGestureRecognizer *)sender
{
    ...
    [[PSActiveState sharedInstance].activeTool gestureBegan:sender];
    [[PSActiveState sharedInstance].activeTool gestureEnded:sender];
}

- (void) handlePanGesture:(PSPanGestureRecognizer *)sender
{
    if (![self canSendTouchToActiveTool]) {
        [self showCannotEditMessage];
        return;
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        [[PSActiveState sharedInstance].activeTool gestureMoved:sender];
    } else if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:WDGestureEndedNotification object:nil];
        currentlyPainting = NO;
        [[PSActiveState sharedInstance].activeTool gestureCanceled:sender];
    }
}
...
{    
    ...
    if (userInfo && userInfo[@"rect"]) {
        ...
        self.dirtyRect = PSUnionRect(self.dirtyRect, rect.CGRectValue);
    } else {
...
- (void) showImageMessage:(NSArray *)images
{
    if (!imageMessageView_) {
        ...
        imageMessageView_.sharpCenter = PSCenterOfRect(self.superview.bounds);
...
{
    [self showMessage:message autoHide:YES position:PSCenterOfRect(self.superview.bounds) duration:kMessageFadeDelay];
}
...
- (void) showMessage:(NSString *)message autoHide:(BOOL)autoHide position:(CGPoint)position duration:(float)duration
{
    ...
    if (!messageLabel_) {
        messageLabel_ = [[PSLabel alloc] initWithFrame:CGRectInset(CGRectMake(0,0,100,40), -8, -8)];
...
@implementation PSCanvas (WDPlacePhotoMode)
...
- (void) photoTransformChanged:(PSTransformOverlay *)sender
{
...
- (void) beginPhotoPlacement:(UIImage *)image
{
    ...
    transformOverlay_ = [[PSTransformOverlay alloc] initWithFrame:self.superview.frame];
    ...
    self.photo = image;
    self.photoTexture = [PSTexture textureWithImage:image];
    transformOverlay_.prompt = @"调整图片到合适的位置";
    transformOverlay_.title = @"调整图片";
    transformOverlay_.alpha = 0.0f; // for fade in
    transformOverlay_.showToolbar = YES;
    transformOverlay_.canvas = self;
    
    __unsafe_unretained PSCanvas *canvas = self;
...
- (void) placePhoto
{
    ...
    BOOL mergeDown = NO;
    changeDocument(painting_, [PSAddImage addImage:photo_ atIndex:index mergeDown:mergeDown transform:photoTransform_]);
...
@implementation PSCanvas (WDLayerTransformMode)

- (void) layerTransformChanged:(PSTransformOverlay *)sender
{
    CGAffineTransform flip = CGAffineTransformIdentity;
    
    if (sender.horizontalFlip || sender.verticalFlip) {
        CGPoint center = PSCenterOfRect(self.painting.bounds);
        flip = CGAffineTransformTranslate(flip, center.x, center.y);
...
- (void) beginLayerTransformation
{
    ...
    transformOverlay_.prompt = @"拖拽调整图层";
    transformOverlay_.title = @"裁剪图层";
...
- (void) transformActiveLayer
{
    ...
    if (!CGAffineTransformIsIdentity(rawLayerTransform)) {
        changeDocument(self.painting, [PSTransformLayer transformLayer:self.painting.activeLayer transform:rawLayerTransform]);
    }
