...
- (BOOL) colorChanged:(PSColor *)color
{
    if (!color) {
        return YES;
    }
...
- (void) setColor:(PSColor *)color
{
    ...
    if (self.reversed) {
        ...
        if (mode_ == WDColorSliderModeHue) {
            color = [PSColor colorWithHue:color.hue saturation:1 brightness:1 alpha:1];
        }
...
{    
    ...
    if (mode_ == WDColorSliderModeAlpha) {
        PSDrawCheckersInRect(ctx, bounds, 8);
    }
