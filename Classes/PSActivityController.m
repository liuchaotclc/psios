...
@implementation PSActivityController
...
- (void) setActivityManager:(PSActivityManager *)am
{
    activityManager = am;
    table.dataSource = am;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(activityAdded:) name:PSActivityAddedNotification object:am];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(activityRemoved:) name:PSActivityRemovedNotification object:am];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(activityProgressChanged:) name:PSActivityProgressChangedNotification object:am];
}
...
{
    NSNumber    *index = (aNotification.userInfo)[@"index"];
    PSActivity  *activity = (aNotification.userInfo)[@"activity"];
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:index.integerValue inSection:0];
