...
#import "PSThumbButton.h"
#import "UIImage+Additions.h"
#import "UIView+Additions.h"

@interface PSThumbnailView (Private)
- (void) reloadFilenameFields_;
@end

@implementation PSThumbnailView
...
- (void) setSelected:(BOOL)flag
{
    ...
    if (flag) {
        ...
        mySize = PSMultiplyPointScalar(mySize, 0.5f);
        CGPoint center = PsSubtractPoints(topRightCorner, mySize);
...
- (void) textEditingDidEnd:(id)sender
{
    ...
    if (newName.length == 0) {
        ...
        errorMessage = @"The painting name cannot begin with a dot “.”.";
    } else if ([newName rangeOfString:@"/"].length > 0 || [newName rangeOfString:@":"].length > 0) {
        errorMessage = @"The painting name cannot contain “:” or “/”.";
    } else if ([[PSPaintingManager sharedInstance] paintingExists:newName]) {
        NSString *format = @"A painting with the name “%@” already exists. Please choose a different name.";
        errorMessage = [NSString stringWithFormat:format, newName];
    } else {
        [[PSPaintingManager sharedInstance] renamePainting:filename_ newName:titleField_.text];
    }
...
{
    if (!thumbButton_) {
        thumbButton_ = [PSThumbButton thumbButtonWithFrame:self.bounds];
    ...
    [[PSPaintingManager sharedInstance] getThumbnail:filename_ withHandler:^void(UIImage *image) {
...
{
    if (self.superview) {
        UIActivityIndicatorViewStyle style = PSDeviceIsPhone() ? UIActivityIndicatorViewStyleWhite : UIActivityIndicatorViewStyleWhiteLarge;
        ...
        float inset = PSDeviceIsPhone() ? -5 : -10;
        ...
        activityView_.sharpCenter = PSCenterOfRect(bgView.bounds);
        bgView.sharpCenter = PSCenterOfRect(self.bounds);
...
@implementation PSThumbnailView (Private)
