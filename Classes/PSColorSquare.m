...
#import "PSInsetView.h"
#import "PSShader.h"
#import "PSUtilities.h"
#import "UIView+Additions.h"
#import "gl_matrix.h"

@implementation PSColorSquare
...
{    
    ...
    PSInsetView *insetView = [[PSInsetView alloc] initWithFrame:self.bounds];
    [self addSubview:insetView];
    
    indicator_ = [[PSColorIndicator alloc] initWithFrame:CGRectMake(0,0,24,24)];
    indicator_.sharpCenter = PSCenterOfRect(self.bounds);
...
- (PSShader *) colorShader
{
    if (!colorShader_) {
        NSArray *attributes = @[@"inPosition", @"inTexcoord"];
        NSArray *uniforms = @[@"modelViewProjectionMatrix", @"hue"];
        
        colorShader_ = [[PSShader alloc] initWithVertexShader:@"blit" fragmentShader:@"colorPicker"
...
- (void) drawView
{
    ...
    // use shader program
    PSShader *colorShader = self.colorShader;
...
- (CGPoint) indicatorPosition
{
    ...
    result = PSAddPoints(result, self.trackRect.origin);
