...
- (id) initWithProgress:(PSCodingProgress *)progress
{
    self = [super init];
    if (!self) {
        return nil;
...
- (NSDictionary *) dataWithRootNamed:(NSString *)name
{
    ...
    [copy setValue:[PSTypedData data:[self jsonData] mediaType:@"application/json"] forKey:name];
    return copy;
}
...
- (void) encodeObject:(id<PSCoding>)object forKey:(NSString *)key deep:(BOOL)deep 
{
    ...
    if (object) {
        ...
        [json_ appendString:@"\""];
        [object encodeWithPSCoder:self deep:deep];
        [json_ appendString:@"}"];
#if WD_DEBUG
...
- (void) encodeData:(NSData *)data forKey:(NSString *)key mediaType:(NSString *)mediaType 
{
    [self encodeKey:key];
    if ([data length] > 256) {
        NSString *extension = [PSJSONCoder extensionForType:mediaType];
        NSString *filename = [self uniqueFilenameWithPrefix:key extension:extension];
        [binary_ setValue:[PSTypedData data:data mediaType:mediaType] forKey:filename];
        [json_ appendString:@"{"];
...
- (void) encodeUnknown:(id)object forKey:(NSString *)key {
    ...
    } else if ([object conformsToProtocol:@protocol(PSCoding)]) {
    ...
    } else if ([object conformsToProtocol:@protocol(PSDataProvider)]) {
...
- (id) reconstructDictionary:(NSDictionary *)dict binary:(NSDictionary *)binary 
{
        ...
        case 'd':
            if ([type isEqualToString:@T_DATA]) {
                ...
                if (data) {
                    ...
                    if ([value conformsToProtocol:@protocol(PSDataProvider)]) {
                        return value;
                    } else if ([value isKindOfClass:[NSData class]]) {
                        return [PSTypedData data:value mediaType:type];
                    } else if (value) {
                        WDLog(@"ERROR: Binary data of unknown type! src: %@ type: %@", src, [value class]);
                        return [PSTypedData data:[NSData data] mediaType:type];
                    } else {
    ...
    if ([class conformsToProtocol:@protocol(PSCoding)]) {
        id<PSCoding> obj = [[class alloc] init];
        PSDictionaryDecoder *decoder = [[PSDictionaryDecoder alloc] initWithDictionary:converted progress:progress_];
        [obj updateWithPSDecoder:decoder deep:YES];
...
+ (NSData *) dataFrom:(id)obj
{
    if ([obj isKindOfClass:[NSData class]]) {
        return obj;
    } else if ([obj conformsToProtocol:@protocol(PSDataProvider)]) {
        return [obj data];
    } else {
...
{
    NSError *err;
    NSData *jsonSource = [PSJSONCoder dataFrom:[source valueForKey:name]];
...
- (id<PSCoding>) copy:(id <PSCoding>)source deep:(BOOL)deep 
{
    ...
    id<PSCoding> copy = [coder decodeRoot:root from:data];
    return copy;
}
...
- (void) update:(id <PSCoding>)dest with:(id <PSCoding>)source 
{
    ...
    NSError *err;
    NSData *jsonSource = [PSJSONCoder dataFrom:coder.jsonData];
    ...
    PSDictionaryDecoder *decoder = [[PSDictionaryDecoder alloc] initWithDictionary:converted progress:progress_];
    [dest updateWithPSDecoder:decoder deep:NO];
    [decoder waitForQueue];
}
