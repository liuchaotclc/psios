...
{
    PSMenuItem *item = [[PSMenuItem alloc] init];
...
{
    PSMenuItem *item = [[PSMenuItem alloc] initWithTitle:aString image:nil action:aSelector target:target];
    return item;
}

+ (id)itemWithTitle:(NSString *)aString image:(UIImage *)image action:(SEL)aSelector target:(id)target
{
    PSMenuItem *item = [[PSMenuItem alloc] initWithTitle:aString image:image action:aSelector target:target];
    return item;
}
