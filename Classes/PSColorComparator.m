...
@implementation PSColorComparator
...
- (void) awakeFromNib 
{
    initialColor_ = [PSColor whiteColor];
    currentColor_ = [PSColor whiteColor];
...
- (void) paintTransparentColor:(PSColor *)color inRect:(CGRect)rect
{
    CGContextRef    ctx = UIGraphicsGetCurrentContext();
    ...
    PSDrawCheckersInRect(ctx, rect, 8);
    [[color UIColor] set];
...
{
    [self setCurrentColor:(PSColor *)[sender color]];
}

- (void) setCurrentColor:(PSColor *)color
{
    currentColor_ = color;
...
- (void) setOldColor:(PSColor *)color
{
    initialColor_ = color;
