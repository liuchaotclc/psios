...
@implementation PSTransformOverlay
...
- (CGAffineTransform) configureInitialPhotoTransform
{
    ...
    CGPoint offset = PsSubtractPoints(paintingCenter, photoCenter);
    transform_ = CGAffineTransformIdentity;
...
- (IBAction)handlePanGesture:(UIPanGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateChanged) {
        ...
        loc = PSMultiplyPointScalar(loc, 1.0f / canvas.scale * [UIScreen mainScreen].scale);
...
{
    UIBarButtonItem *flipHorizontally = [[UIBarButtonItem alloc] initWithTitle:@"水平翻转"
                                                               style:UIBarButtonItemStyleBordered
                                                              target:self action:@selector(flipHorizontally:)];
                               
    UIBarButtonItem *flipVertically = [[UIBarButtonItem alloc] initWithTitle:@"垂直翻转"
                                                               style:UIBarButtonItemStyleBordered
...
- (void) buildNavBar
{
    ...
    CGRect frame = self.bounds;
    frame.size.height = 44;
    navbar_ = [[UIToolbar alloc] initWithFrame:frame];
...
//    UINavigationItem *navItem = [[UINavigationItem alloc] initWithTitle:self.title];
    ...
    UIBarButtonItem *accept = [[UIBarButtonItem alloc] initWithTitle:@"确定"
                                                               style:UIBarButtonItemStylePlain
                                                              target:self action:@selector(accept:)];
    UIBarButtonItem *flexible = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace
    target:nil action:nil];
    
    NSArray *result = @[cancel, flexible, accept];
    navbar_.items = result;
