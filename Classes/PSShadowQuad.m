...
@interface PSShadowQuad ()
@property (nonatomic) PSTexture *texture;
@end


@implementation PSShadowQuad {
    GLuint vao_;
    GLuint vbo_;
    CGRect lastRect;
}
...
{
    PSShadowQuad *quad = [[PSShadowQuad alloc] init];
...
{
    if (!texture) {
        texture = [PSTexture textureWithImage:self.image];
...
+ (void) configureBlit:(GLfloat *)proj withShader:(PSShader *)blitShader
{
...
- (void) blitWithScale:(float)scale
{
    ...
    if (!CGRectEqualToRect(rect, lastRect)) {
        [self freeBuffers];
        PSGLBuildQuadForRect(rect, CGAffineTransformIdentity, &vao_, &vbo_);
    }
