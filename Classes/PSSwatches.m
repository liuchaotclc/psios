...
- (void) drawSwatchInRect:(CGRect)rect color:(PSColor *)color
{
    if (!color) {
    ...
    for (int y = 0; y < numRows; y++) {
        ...
        for (int x = 0; x < swatchesPerRow; x++) {
            ...
            if (CGRectIntersectsRect(swatch, rect)) {
                swatchColor = [[PSActiveState sharedInstance] swatchAtIndex:index];
...
- (void)dragMoved:(UITouch *)touch colorChip:(PSDragChip *)chip colorSource:(id)colorSource 
{
...
- (BOOL) dragEnded:(UITouch *)touch colorChip:(PSDragChip *)chip colorSource:(id)colorSource destination:(CGPoint *)flyLoc
{
    CGPoint pt = [touch locationInView:self];
    ...
    if (initialIndex_ >= 0) {
        [[PSActiveState sharedInstance] setSwatch:nil atIndex:initialIndex_];
        [self setNeedsDisplayInRect:[self rectForSwatchIndex:initialIndex_]];
    }
    
    NSInteger index = [self indexAtPoint:pt];

    *flyLoc = [self convertPoint:PSCenterOfRect([self rectForSwatchIndex:index]) toView:chip.superview];
    
    [[PSActiveState sharedInstance] setSwatch:[chip color] atIndex:index];
...
- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    self.tappedColor = [[PSActiveState sharedInstance] swatchAtIndex:initialIndex_];
...
{   
    ...
    NSInteger  index = [self indexAtPoint:pt];
    PSColor    *color = [[PSActiveState sharedInstance] swatchAtIndex:index];
    ...
    if (touch.tapCount == 2) {
        ...
        [[PSActiveState sharedInstance] setSwatch:[PSActiveState sharedInstance].paintColor atIndex:index];
        [self setNeedsDisplayInRect:[self rectForSwatchIndex:index]];
    }
